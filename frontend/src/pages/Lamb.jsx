...
const Lamb = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await axios.get('https://way2save.onrender.com/auth/products/Baby Lamb');
                ...
                <div className="container mx-auto px-4 py-16 flex flex-col lg:flex-row items-center gap-8">
                    <div className="relative w-full lg:w-1/2 h-[300px] lg:h-[400px] ml-8 md:ml-14 xl:ml-0">
                        <div className="absolute top-0 left-0 w-[80%] h-full rounded-3xl transform -rotate-6">
                            <img src={Lamb1} className="h-full w-full object-cover rounded-3xl" alt="" />
                        </div>
                        <div className="absolute top-0 left-[10%] w-[80%] h-full rounded-3xl transform rotate-6">
                            <img src={Lamb2} className="h-full w-full object-cover rounded-3xl" alt="" />
                        </div>
                    </div>

                    <div className="w-full lg:w-1/2 space-y-6 text-center lg:text-left">
                        <h1 className="text-4xl lg:text-5xl font-bold text-navy-900 leading-tight font-jost">
                            Fresh Halal Lamb Delights
                        </h1>
                        <p className="text-lg text-gray-600 max-w-lg font-jost">
                            Sourced from trusted halal farms, our lamb is a true reflection of quality and ethical farming practices. From succulent chops to melt-in-your-mouth shoulder roasts, each cut is carefully prepared to offer you a rich, flavorful dining experience. Taste the difference in every tender bite, knowing it's all halal-certified.
                        </p>
                    </div>
                </div>
            </div>
